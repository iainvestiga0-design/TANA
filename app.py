...
GEMINI_MODEL_2 = st.secrets.get("TANA_GEMINI_MODEL_2", os.getenv("TANA_GEMINI_MODEL_2", "gemini-3.5-flash"))
GEMINI_MODEL_3 = st.secrets.get("TANA_GEMINI_MODEL_3", os.getenv("TANA_GEMINI_MODEL_3", "gemini-3.5-flash-lite"))
...
def _generate_with_fallback(contents_factory, config):
    ...
    for profile in profiles:
        ...
        # Reintentos automáticos cuando Gemini responde 503 (High Demand)
        for intento in range(3):
            try:
                ...
                if "503" in str(exc).lower() or "high demand" in str(exc).lower():
                    if intento < 2:
...
def extract_with_gemini(uploaded):
    ...
    try:
        ...
        for profile in profiles:
            ...
            for intento in range(3):
                try:
                    # Cada perfil tiene su propio cliente/proyecto.
